...
function Factory:newController()
	...
	function Controller:removeJoystick(name)
		if self.objects[name]==nil then return end
		display.remove(self.objects[name].touchZone)
		display.remove(self.objects[name].container)
		display.remove(self.objects[name].nob)
